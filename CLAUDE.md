...
- `backend/ai_interviewer.db` is checked in (dev convenience). Schema changes via `init_db()` only `create_all` — they will not migrate existing tables, so deleting the file is the standard way to pick up model changes locally.
